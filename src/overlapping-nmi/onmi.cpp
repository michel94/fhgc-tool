...
#include "cmdline.h"
#include "../webgraph/clusterUtils.hpp"
...
double oNMI(Grouping &g1, Grouping &g2, const bool do_omega_also);

long double calcONMI(const char* file1, const char* file2){
	...
	Grouping g1(clusters1.size()), g2(clusters2.size());

	for(int i=0; i<(int)clusters1.size(); i++){
		for(int j=0; j<(int)clusters1[i]->size(); j++){
			g1[i].insert( to_string((*clusters1[i])[j]) );
		}
	}
	for(int i=0; i<(int)clusters2.size(); i++){
		for(int j=0; j<(int)clusters2[i]->size(); j++){
			g2[i].insert( to_string((*clusters2[i])[j]) );
		}
	}
	return oNMI(g2, g1, false);
}
...
double oNMI(Grouping &g1, Grouping &g2, const bool do_omega_also) {
	...
	double res = aaronNMI<Max>(om, omFlipped, g1, g2);

	/*cerr << "NMI<Max>:\t"; cerr << res << endl;
	cerr << "Other measures:" << endl;
	cerr << "  lfkNMI:\t"; cerr << LFKnmi_ << endl;
	cerr << "  NMI<Sum>:\t"; cerr << aaronNMI<Sum>(om, omFlipped, g1, g2) << endl;*/
