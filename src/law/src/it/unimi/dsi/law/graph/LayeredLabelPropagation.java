...
public class LayeredLabelPropagation {
	...
	private final class IterationThread extends Thread {
		@SuppressWarnings("hiding")
		private ImmutableGraph symGraph;
		...
		public void run() {
			...
			final XorShift128PlusRandom r = new XorShift128PlusRandom( LayeredLabelPropagation.this.seed );
			...
			for ( ;; ) {
				...
				for ( int i = start; i < end; i++ ) {
					...
					if ( canChange[ node ] ) {
						...
						int outdegree = symGraph.outdegree( node );
						if ( outdegree > 0 ) {
							...
							for ( Iterator<Int2IntMap.Entry> entries = map.entries(); entries.hasNext(); ) {
								Int2IntMap.Entry entry = entries.next();
							...
							if ( exact ) {
								if ( startPerm != null ) IntArrays.quickSort( majorities.elements(), 0, majorities.size(), new AbstractIntComparator() {
									public int compare( final int a, final int b ) {
										return startPerm[ a ] - startPerm[ b ];
									}
								} );
	...
	public AtomicIntegerArray computeLabels( final double gamma, int maxUpdates ) {
		init();
		String gammaFormatted = GAMMA_FORMAT.format( gamma );
		double prevObjFun = 0;
		double gain = 0;
	 ...
	 * @return the final permutation of the graph.
	 */
	public int[] computePermutation( final double[] gammas, final String cluster, int maxUpdates ) throws IOException {
		final int n = this.n;
		final int m = gammas.length;
		System.out.println("Number of gammas: " + m);
		...
		for ( int index = 0; index < m; index++ ) {
			...
			do {
				prevObjFun = objectiveFunction();
				update( gamma ); // TODO: Get thread number to check where parallelization begins
				pl.updateAndDisplay();
				gain = 1 - ( prevObjFun / objectiveFunction() );
				LOGGER.info( "Gain: " + gain );
				LOGGER.info( "Modified: " + modified.get() );
			...
			final Writer bos = new BufferedWriter(new OutputStreamWriter(new FileOutputStream(labelling + "-" + index + ".ascii")));
	...
	public static void main( String[] args ) throws IOException, JSAPException {
		...
		DoubleArrayList gammas = new DoubleArrayList();
		for ( String gamma : jsapResult.getString( "gammas" ).split( "," ) ) {
