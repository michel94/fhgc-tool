...
/*
 * Copyright (C) 2013-2015 Paolo Boldi, Massimo Santini and Sebastiano Vigna
 *
...
/** Computes the weighted &tau; between two score vectors. More precisely, the this class computes the formula given by
 * Sebastiano Vigna in &ldquo;<a href="http://vigna.di.unimi.it/papers.php#VigWCIRT">A weighted correlation index for rankings 
 * with ties</a>&rdquo;, 2014, using the algorithm therein described (see details below). 
 * 
...
public class WeightedTau extends CorrelationIndex {
	...
	public double compute( final double v0[], double v1[], int[] rank ) {
		...
		tot = multiplicative ? ( s * s - sq ) / 2 : s * ( length - 1 );
